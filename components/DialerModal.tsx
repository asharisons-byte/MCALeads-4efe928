...
export const DialerModal: React.FC<DialerModalProps> = ({
  ...
  // Resume remote audio when call becomes CONNECTED (handles autoplay policy)
  useEffect(() => {
    if (callState === 'CONNECTED' && remoteAudioRef.current) {
      const audio = remoteAudioRef.current;
      if (audio.paused && audio.srcObject) {
        audio.play().catch(e =>
          console.warn('[MCA] Remote audio play() blocked:', e.message)
        );
      }
    }
  }, [callState]);
  ...
  const handleToggleMute = async () => {
    ...
    setIsMuted(newMute);
    if (isWebRTCConnected) {
      if (newMute) await TelnyxWebRTCService.mute();
      else await TelnyxWebRTCService.unmute();
    } else if (activeCallRecord) {
  ...
  const handleToggleHold = async () => {
    ...
    if (isWebRTCConnected) {
      if (newHold) await TelnyxWebRTCService.hold();
      else await TelnyxWebRTCService.unhold();
    } else if (activeCallRecord) {
          ...
          <div className="lg:col-span-5 flex flex-col min-h-0 bg-[#0c101d] overflow-y-auto p-5 space-y-5">
            ...
            {showKeypad && (
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 shadow-inner">
                ...
                {(callState === 'IDLE' || callState === 'FAILED' || callState === 'COMPLETED') && phoneNumber && (
            ...
            <div className="p-5 rounded-2xl bg-gradient-to-b from-[#101729] to-[#0d1222] border border-slate-800 text-center space-y-2">
              <div className="flex items-center justify-center gap-2">
                {callState === 'CONNECTED' ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-500/20 text-orange-300 text-xs font-bold border border-orange-500/30">
                    <AlertTriangle className="w-3 h-3" />
                    Line Busy
                  </span>
                ) : callState === 'FAILED' ? (
            ...
            <div className="flex items-center justify-center gap-3">
              {callState === 'IDLE' || callState === 'FAILED' ? (
              ...
              ) : callState === 'COMPLETED' || callState === 'NO_ANSWER' || callState === 'BUSY' ? (
                <div className="w-full flex items-center gap-3">
                  <button
                    ...
                    className="flex-1 py-3 px-4 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-sm font-bold flex items-center justify-center gap-2 border border-slate-700 transition-all"
                  >
                  ...
                  <button
                    onClick={() => {
                      ...
                      setActiveCallRecord(null);
                      setIsWebRTCConnected(false);
                      setTimeout(() => handleStartCall(), 150);
                    }}
                    disabled={!hasPhoneNumber}
                    className="flex-1 py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white text-sm font-bold flex items-center justify-center gap-2 shadow-lg shadow-emerald-600/20 transition-all active:scale-[0.98]"
                  >
                    <Phone className="w-4 h-4" />
                    <span>Redial</span>
                  </button>
