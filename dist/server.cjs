...
var inMemoryAiContent = [];
var inMemoryWebhookEvents = [];
var inMemoryAutomationRuns = [];
var inMemoryIntegrations = [
  { id: 1, provider: "Google Gemini AI", integrationType: "AI Engine", status: "Active", configurationReference: { model: "gemini-3.8-flash" }, lastSyncAt: /* @__PURE__ */ new Date() },
  { id: 2, provider: "Telnyx Voice", integrationType: "Telephony", status: "Active", configurationReference: { webrtc: true, sip: true }, lastSyncAt: /* @__PURE__ */ new Date() },
  { id: 3, provider: "Telnyx SMS", integrationType: "Messaging", status: "Active", configurationReference: { messagingProfileId: "active" }, lastSyncAt: /* @__PURE__ */ new Date() },
...
async function updateDbLead(leadId, updates) {
  ...
  if (target) {
    ...
    if (updates.assigned_to !== void 0) target.assignedTo = updates.assigned_to;
    if (updates.assignedTo !== void 0) target.assignedTo = updates.assignedTo;
...
async function getDbDashboardMetrics() {
  ...
  const confirmedMrr = inMemoryClients.filter((c) => c.clientStatus === "Active").reduce((sum, c) => sum + Number(c.actualMrr || 0), 0);
  const pipelineStages = /* @__PURE__ */ new Set(["Negotiation", "Proposal Sent", "Audit Sent"]);
  const pipelineMrr = activeLeads.filter((l) => pipelineStages.has(l.leadStatus)).reduce((sum, l) => sum + Number(l.estimatedRetainer || 0), 0);
...
async function getDbAiContentForLead(leadId) {
  ...
  if (isDbConfigured) {
    try {
      const dbRows = await db.select().from(aiContent).where((0, import_drizzle_orm2.eq)(aiContent.leadId, existing.id)).orderBy((0, import_drizzle_orm2.desc)(aiContent.createdAt));
...
async function recordDbWebhookEvent(params) {
  const event = {
    id: Math.floor(Math.random() * 9e5) + 1e5,
  ...
  if (isDbConfigured) {
    try {
      const [dbEvent] = await db.insert(webhookEvents).values(event).returning();
      return dbEvent;
    } catch (error) {
    }
  }
...
async function getDbWebhookEvents(limit = 50) {
  if (isDbConfigured) {
    try {
      const rows = await db.select().from(webhookEvents).orderBy((0, import_drizzle_orm2.desc)(webhookEvents.createdAt)).limit(limit);
      if (rows && rows.length > 0) return rows;
    } catch (error) {
    }
  }
...
async function recordDbAutomationRun(params) {
  const run = {
    id: Math.floor(Math.random() * 9e5) + 1e5,
  ...
  if (isDbConfigured) {
    try {
      const [dbRun] = await db.insert(automationRuns).values(run).returning();
      return dbRun;
    } catch (error) {
    }
  }
...
async function updateDbAutomationRun(runId, updates) {
  ...
  if (isDbConfigured) {
    try {
      ...
      }).where((0, import_drizzle_orm2.eq)(automationRuns.id, Number(runId))).returning();
      return updated;
    } catch (error) {
    }
  }
...
async function getDbAutomationRuns(limit = 50) {
  if (isDbConfigured) {
    try {
      const rows = await db.select().from(automationRuns).orderBy((0, import_drizzle_orm2.desc)(automationRuns.createdAt)).limit(limit);
      if (rows && rows.length > 0) return rows;
    } catch (error) {
    }
  }
...
async function findLeadByPhone(phone) {
  ...
  if (isDbConfigured) {
    try {
      const candidates = await db.select().from(leads).where(import_drizzle_orm2.sql`${leads.phone} ILIKE ${"%" + last7 + "%"}`).limit(1);
...
async function recordDbInboundSms(params) {
  ...
  if (isDbConfigured) {
    try {
      const [dbSms] = await db.insert(smsMessages).values({
        leadId,
        phone: params.phone,
        message: params.message,
        direction: "Inbound",
...
async function getDbIntegrations() {
  if (isDbConfigured) {
    try {
      const configuredRows = await db.select().from(integrations);
...
async function updateDbIntegrationStatus(provider, integrationType, status, configurationReference = {}) {
  ...
  if (existingMem) {
    ...
    existingMem.lastSyncAt = /* @__PURE__ */ new Date();
  } else {
  ...
  if (isDbConfigured) {
    try {
      const existing = await db.select().from(integrations).where((0, import_drizzle_orm2.eq)(integrations.provider, provider)).limit(1);
      if (existing.length > 0) {
        const [updated] = await db.update(integrations).set({ status, configurationReference, lastSyncAt: /* @__PURE__ */ new Date() }).where((0, import_drizzle_orm2.eq)(integrations.id, existing[0].id)).returning();
        return updated;
      } else {
...
function maskSecret(val, visibleChars = 4) {
  if (!val) return "Not Configured";
  if (val.length <= visibleChars * 2) return "\u2022\u2022\u2022\u2022\u2022\u2022\u2022\u2022";
  return `${val.substring(0, visibleChars)}\u2022\u2022\u2022\u2022${val.substring(val.length - visibleChars)}`;
}
var inMemoryWebhookEvents2 = [];
var inMemoryAutomationRuns2 = [];
function recordLocalWebhookEvent(event) {
  inMemoryWebhookEvents2.unshift({
    id: event.id || Math.floor(Math.random() * 9e5) + 1e5,
  ...
  if (inMemoryWebhookEvents2.length > 100) inMemoryWebhookEvents2.pop();
}
function recordLocalAutomationRun(run) {
  inMemoryAutomationRuns2.unshift({
    id: run.id || Math.floor(Math.random() * 9e5) + 1e5,
  ...
  if (inMemoryAutomationRuns2.length > 100) inMemoryAutomationRuns2.pop();
}
router2.get("/integrations/status", async (req, res) => {
  try {
    ...
    try {
      await db.select({ ping: import_drizzle_orm3.sql`1` });
...
router2.post("/integrations/test/:service", async (req, res) => {
  ...
  try {
    switch (service) {
      ...
      case "database": {
        ...
        const dbDetails = getDatabaseDetails();
        try {
          const [leadCount] = await db.select({ count: import_drizzle_orm3.sql`count(*)` }).from(leads);
      ...
      case "n8n": {
        ...
        try {
          ...
          return res.json({
            ...
            message: n8nRes.ok ? "n8n workflow webhook endpoint responded successfully!" : `n8n webhook returned status ${n8nRes.status}. Check endpoint URL and active state in n8n canvas.`
          });
        } catch (netErr) {
          return res.json({
            success: false,
...
router2.post("/webhooks/telnyx/voice", async (req, res) => {
  try {
      ...
      switch (eventType) {
        ...
        case "call.recording.saved": {
          ...
          if (recordingUrl && callControlId) {
            await db.update(calls).set({ recordingUrl }).where((0, import_drizzle_orm3.eq)(calls.externalCallId, callControlId));
          }
          break;
        }
        case "call.transcription.saved": {
          ...
          if (transcription && callControlId) {
            await db.update(calls).set({ transcript: transcription }).where((0, import_drizzle_orm3.eq)(calls.externalCallId, callControlId));
            const ai = getGeminiClient();
            if (ai) {
              try {
...
${transcription}`
                ...
                if (summaryResp.text) {
                  await db.update(calls).set({ aiSummary: summaryResp.text }).where((0, import_drizzle_orm3.eq)(calls.externalCallId, callControlId));
                }
              } catch (err) {
...
router2.post("/webhooks/telnyx/sms", async (req, res) => {
  try {
    const event = req.body;
    ...
    if (eventType === "message.received") {
      ...
      let aiClassification = { intent: "Inquiry", recommended_next_action: "Reply to confirm details" };
      const ai = getGeminiClient();
      if (ai && text2) {
        try {
...
Return JSON:
{
  ...
  "suggested_reply": "A concise, polite, professional SMS response (under 140 chars) from Sophia"
          ...
          if (text2.toLowerCase().includes("stop") || text2.toLowerCase().includes("unsubscribe")) {
            aiClassification = { intent: "Opt-Out", recommended_next_action: "Mark Do Not Contact" };
          } else if (text2.toLowerCase().includes("yes") || text2.toLowerCase().includes("sure") || text2.toLowerCase().includes("call me")) {
...
router2.post("/integrations/sms/send", async (req, res) => {
  try {
...
router2.post("/integrations/gmail/send", async (req, res) => {
  try {
...
router2.post("/integrations/n8n/trigger", async (req, res) => {
  try {
    ...
    const run = await recordDbAutomationRun({
      ...
      relatedEntityId: leadId ? String(leadId) : void 0,
      status: "DISPATCHED"
    });
...
router2.post("/webhooks/n8n", async (req, res) => {
  try {
    ...
    if (runId) {
      ...
      const memRun = inMemoryAutomationRuns2.find((r) => r.id === Number(runId) || r.externalRunId === String(runId));
      if (memRun) {
        memRun.status = status || "SUCCESS";
        memRun.completedAt = (/* @__PURE__ */ new Date()).toISOString();
...
router2.get("/integrations/webhooks/history", async (req, res) => {
  try {
    const dbEvents = await getDbWebhookEvents(50);
    const seen = /* @__PURE__ */ new Set();
    const combined = [...inMemoryWebhookEvents2, ...dbEvents].filter((ev) => {
    ...
    return res.json({ success: true, events: inMemoryWebhookEvents2 });
  }
});
router2.get("/integrations/n8n/runs", async (req, res) => {
  try {
    const dbRuns = await getDbAutomationRuns(50);
    const seen = /* @__PURE__ */ new Set();
    const combined = [...inMemoryAutomationRuns2, ...dbRuns].filter((r) => {
    ...
    return res.json({ success: true, runs: inMemoryAutomationRuns2 });
  }
});
router2.get("/integrations/ai-content/:leadId", async (req, res) => {
  try {
...
async function generateAiContent(ai, params) {
  ...
  try {
    const resp = await tryCall("gemini-3.8-flash", {
      ...config,
      thinkingConfig: { thinkingLevel: import_genai2.ThinkingLevel.LOW }
    }, 8e3);
...
function getGemini() {
  if (!geminiClient && process.env.GEMINI_API_KEY) {
    geminiClient = new import_genai2.GoogleGenAI({
      apiKey: process.env.GEMINI_API_KEY,
      httpOptions: {
        headers: {
